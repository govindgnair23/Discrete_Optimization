...
function assign_vehicles(demand, coordinates,N_customers;chart=false)
    ...
    demand_ = demand2[order_customers]
    ...
    while customer_counter < N_customers - 1 # depot 0 is not a customer
        ...
        cum_assigned += sum(demand2[assigned_customers])
...
###############Use local search to generate a feasible solution####################

##Steps
#1) Possible actions for a given sector are:
    #a) Do nothing (50%) or reassign (50%)
    #b) Pick a city and assign clockwise (50%)
    #c) Pick a city and assign counterclockwise (50%)
#2) Assign all unassigned cities to a slack vehicle
#3) Minimize Cost = Sum of slack of eligible vehicles + occupied of slack vehicle. 

"""
Function to carry out a local move on the specified sector
"""
function local_move(sector::Int64,N_vehicles::Int64,vehicle_assignment)
    if rand() < 0.25; return vehicle_assignment;end; # DO nothing in this case
    #Get destinations
    clock,counter_clock = sector-1 ,sector+1
    if sector == 1
         clock,counter_clock= N_vehicles+1,2
    end
    if sector == N_vehicles+1
        clock,counter_clock= N_vehicles-1,1
    end
    # Select city to reassign
    selected_city =  sample(vehicle_assignment[sector])
    new_assignment = deepcopy(vehicle_assignment)
    deleteat!(new_assignment[sector],findall(x->x==selected_city,new_assignment[sector]))#remove selected city
    if rand()< 0.5 #move counter clockwise
        push!(new_assignment[counter_clock],selected_city)
    else
        push!(new_assignment[clock],selected_city)
    end
...
"""
Function to evaluate a specific assignment
"""
function evaluate_assignment(vehicle_assignment,demand,N_vehicles,vehicle_capacity)
    demand_ = demand[2:end]
    load = 0
    for vehicle ∈ keys(vehicle_assignment)
        load += sum(demand_[vehicle_assignment[vehicle]])
    end
    cost = N_vehicles*vehicle_capacity - load #Slack across vehicles
    cost += sum(vehicle_assignment[N_vehicles+1]) # Add assignment to slack vhicles
    return cost
end


"""
Function to evaluate feasibility of an assignment
"""
function evaluate_feasibility(vehicle_assignment,demand,N_vehicles,vehicle_capacity)
    demand_ = demand[2:end]
    load = 0
    for vehicle ∈ keys(vehicle_assignment)
        load = sum(demand_[vehicle_assignment[vehicle]])
        load > vehicle_capacity && return false
    end
    #Get load on slack vehicle
    slack_load =  sum(demand_[vehicle_assignment[N_vehicles+1]])
    slack_load > 0 && return false
    return true
end

"""
Function to do local search by looping through each sector
"""
function local_search(vehicle_assignment,N_vehicles,demand,vehicle_capacity)
    sectors = collect(keys(vehicle_assignment))
    original_cost = evaluate_assignment(vehicle_assignment,demand,N_vehicles,vehicle_capacity)
    for sector ∈ sectors #There are as many sectors as N_vehicles
        vehicle_assignment = local_move(sector,N_vehicles,vehicle_assignment)
    end
    new_cost = evaluate_assignment(vehicle_assignment,demand,N_vehicles,vehicle_capacity)
    is_feasible = evaluate_feasibility(vehicle_assignment,demand,N_vehicles,vehicle_capacity)
    if new_cost <original_cost
        return vehicle_assignment,new_cost-original_cost,is_feasible
    end
end

vehicle_assignment,_,_ = local_search(vehicle_assignment,N_vehicles,demand,vehicle_capacity)
...
function vehicle_routing(vehicle_assignment,coordinates)
    ...
    for vehicle ∈ vehicles
        ...
        cities = vehicle_assignment[vehicle]
        if length(cities) == 1
