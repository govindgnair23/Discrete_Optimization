...
# If no improvement, restart from a new greedy solution
# Use intensification to keep track of a high quality solution
f = open(input_file)
...
function random_swap(route,route_distance)
    ...
    return new_route, route_distance-discarded_edge_distance+new_edge_distance
...
function random_swap_N(route,route_distance,N::Int64)
    map(1:N) do _ 
          route, route_distance = random_swap(route,route_distance)
    end
...
#optimal_route , optimal_distance =  two_opt_exhaustive(optimal_route,optimal_distance)
#optimal_route , optimal_distance = simulated_annealing(coordinates,budget=10000,T=3.5,schedule=0.99,reset_point=100) #434.94
optimal_route , optimal_distance = simulated_annealing(coordinates,budget=1000,T=5.0,schedule=0.99,reset_point=10)
#optimal_route , optimal_distance = simulated_annealing(coordinates,budget=10000,T=5.0,schedule=0.9,reset_point=5)
