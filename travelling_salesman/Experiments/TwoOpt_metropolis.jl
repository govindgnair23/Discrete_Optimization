...
# Implement the 2-Opt technique with metropolis sampling
f = open(input_file)
...
T:: temperature
"""
function two_opt_metro(route::Vector{Int64},route_distance::Float64,T::Float64) # To do: pass distance as an argument so as not to recompute route distance each time
    best_route = route
    best_route_distance = route_distance    
    N_cities = length(route)

    for i ∈ rand(1:N_cities-1)
        for k ∈ rand(i+1:N_cities)
    ...
    return two_opt_metro(best_route,best_route_distance,T)

    
end


route = shuffle!(collect(1:N_cities))
route_distance = calc_distance(route)


optimal_route , optimal_distance =  two_opt_metro(route,route_distance,0.3)
